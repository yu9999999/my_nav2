...
bool ControllerServer::isSameDirect(){
  ...
  // 从TransformStamped消息中提取四元数  
  geometry_msgs::msg::Quaternion q = t.transform.rotation;  
  // 将四元数转换为Eigen::Quaterniond，以便更容易地计算偏航角  
  Eigen::Quaterniond eigen_q(q.w, q.x, q.y, q.z);
  // Eigen提供了从四元数直接获取yaw的方法，但这里我们手动计算,二维平面，所以计算yaw即可
  ...
  // RCLCPP_INFO(rclcpp::get_logger("test"),"goal pose: %f, %f",targetPoint.x, targetPoint.y);
  ...
  // RCLCPP_INFO(rclcpp::get_logger("test"),"angleRad: %f, diff: %f",angleRad, difference);
